  ...
  <NCard :bordered="false" class="h-full zkz-layout-shadow">
    ...
    <Modal v-model:show="data.visible" :loading="data.sureLoading" :title="data.title" @close="close" @confirm="sure">
      <NForm ref="formRef" :model="data.dataForm" :rules="data.rules">
        <NFormItem :label="t('pages.sys.organization.name')" path="name">
          <NInput
            v-model:value="data.dataForm.name"
            clearable
            :maxlength="50"
            :placeholder="t('common.inputPlz', { name: t('pages.sys.organization.name') })" />
        </NFormItem>
        <NFormItem :label="t('pages.sys.organization.alias')" path="alias">
          <NInput
            v-model:value="data.dataForm.alias"
            clearable
            :maxlength="50"
            :placeholder="t('common.inputPlz', { name: t('pages.sys.organization.alias') })" />
        </NFormItem>
        <NFormItem :label="t('pages.sys.organization.des')" path="des">
          <NInput
            v-model:value="data.dataForm.des"
            clearable
            :maxlength="500"
            :placeholder="t('common.inputPlz', { name: t('pages.sys.organization.des') })"
            type="textarea" />
        </NFormItem>
      </NForm>
    </Modal>
...
<script setup lang="ts">
  ...
  const data = reactive<PageData<Organization, true>>({
    dataSource: [],
    params: {
      page: {},
      query: {}
